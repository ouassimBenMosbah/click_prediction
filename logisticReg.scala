...
def preprocess(df1: DataFrame, t: Boolean): DataFrame = {
  ...
  val df2 = if (t) 
    {df1.withColumn("bidfloor", when($"bidfloor".isNull, 3).otherwise($"bidfloor")).withColumn("label", when($"label" === true, 1).otherwise(0)).withColumn("os", os(df1("os"))).withColumn("network", network(df1("network"))).withColumn("timestamp", classificateDate(epochToDateUDF(df1("timestamp"))))} 
  else 
    {df1.withColumn("bidfloor", when($"bidfloor".isNull, 3).otherwise($"bidfloor")).withColumn("os", os(df1("os"))).withColumn("network", network(df1("network"))).withColumn("timestamp", classificateDate(epochToDateUDF(df1("timestamp"))))}
